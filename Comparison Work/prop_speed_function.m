...
for ii = 1:2
    im_dir = sprintf('C:\\Users\\Joseph Mockler\\Documents\\GitHub\\2025_Hypersonic_BL_ID\\Comparison Work\\Run33_data');    
    ref_range = img_no(ii)-no_ref:img_no(ii)+no_ref; % reference range
    nn=0;
    for KK = 1:length(ref_range)
        im_file = sprintf('%s\\Img%06d.tif',im_dir,20);
        I = imread(im_file);
        if KK == 1
            IM = double(I);
        else
    ...
    im_file = sprintf('%s\\Img%06d.tif',im_dir,img_no(ii)); %changed directory **RK
    I = imread(im_file);
