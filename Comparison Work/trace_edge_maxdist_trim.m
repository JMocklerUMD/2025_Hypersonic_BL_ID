function [x_seg,y_seg] = ...
    trace_edge_maxdist_trim(I,x_start,y_start,start_dir,orientation,max_distance,trim)
...
%     end
    ...
    for jj = 1:no_dirs
        ...
        if x_new == 0 || y_new == 0 % make sure they're not zero
            finished = 1;
            break
        end
        if I(x_new,y_new) == 1 && ~(sum(x_seg==x_new) > 0 && sum(y_seg==y_new) > 0) % protect against circles
            ...
            finished = 0;
            break
        elseif (sum(x_seg==x_new) > 0 && sum(y_seg==y_new) > 0) && I(x_new,y_new) == 1
            yinds = find(y_seg == y_new);
            xinds = find(x_seg == x_new);
            for xind = xinds
                if sum(yinds==xind) >= 1
                    x_seg = [];
                    y_seg = [];
    ...
    distance = sqrt((x_old - x_start)^2 + (y_old - y_start)^2);
